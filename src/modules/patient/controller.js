...
export const addPatient = asyncHandler(async (req, res, next) => {
  const { name, age, gender, phone, address, medicalCondition, clinicId } = req.body;
...
export const getPatients = asyncHandler(async (req, res, next) => {
  const patients = await Patient.find().populate("clinicId", "name code").populate("queueTicket", "ticketNumber ticketCode status queueNumber");
